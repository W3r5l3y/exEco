...
class BingameTestCase(TestCase):
    def setUp(self):
        ...
        self.bin1 = Bins.objects.create(
            bin_name="Plastic", bin_image="/img/bins/plastic.png"
        )

        self.bin2 = Bins.objects.create(
            bin_name="Glass", bin_image="/img/bins/glass.png"
        )

        self.bin3 = Bins.objects.create(
            bin_name="Card and Paper", bin_image="/img/bins/card_paper.png"
        )

        self.bin4 = Bins.objects.create(
            bin_name="Tins and Cans", bin_image="/img/bins/tins_cans.png"
        )
    ...
    def test_bin_and_item_ids(self):
        ...
        self.assertTrue(
            Bins.objects.filter(bin_id=self.bin1.bin_id).exists(),
            "Bin1 should exist in database",
        )
        self.assertTrue(
            Items.objects.filter(item_id=self.item1.item_id).exists(),
            "Bin2 should exist in database",
        )
    ...
    def test_game_view(self):
        ...
        self.assertTemplateUsed(
            response,
            "bingame/bingame.html",
            "Game view should use the correct template",
        )
        ...
        self.assertIn(
            "bins",
            response.context,
            "Bins should be included in the context (response)",
        )
        self.assertEqual(
            list(response.context["bins"]),
            [self.bin1, self.bin2, self.bin3, self.bin4],
            "The same bins should be in the context (response)",
        )
        ...
        self.assertIn(
            "items",
            response.context,
            "Items should be included in the context (response)",
        )
        self.assertLessEqual(
            len(response.context["items"]),
            6,
            "There should be 6 or less items in the context (response)",
        )
        ...
        for item in response.context["items"]:
            self.assertIsInstance(
                item, Items, "Items should be instances of the Items model"
            )
    ...
    def test_update_leaderboard(self):
        ...
        response = self.client.post(reverse("update_leaderboard"), {"user_score": 10})

        # Check that request is successful
        self.assertEqual(
            response.status_code,
            200,
            "Update leaderboard should return code 200 - indicating success",
        )
        ...
        self.assertEqual(
            user_points.user,
            self.user,
            "User points should be linked to the correct user",
        )
        self.assertEqual(
            user_points.bingame_points,
            10,
            "User Bingame points should be updated to 10",
        )
