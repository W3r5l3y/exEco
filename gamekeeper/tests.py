...
class QRCodeGenerationTests(TestCase):
    ...
    def test_generate_qr_code_success(self):
        ...
        qr_code_path = os.path.join(
            settings.MEDIA_ROOT,
            "qr_codes",
            f"{self.location_data['location_code']}.png",
        )
...
class StravaLinkTests(TestCase):
    ...
    def test_no_strava_links(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response.json(), {"message": "No linked Strava accounts found"}
        )
    ...
    def test_unlink_strava(self):
        ...
        response = self.client.post(
            reverse("unlink_strava", kwargs={"user_id": strava_user.id})
        )
        self.assertEqual(response.status_code, 200)
...
class QRScannerTests(TestCase):
    ...
    def test_enable_qr(self):
        ...
        response = self.client.post(
            reverse("enable_qr", kwargs={"qr_id": location.location_code})
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_disable_qr(self):
        ...
        response = self.client.post(
            reverse("disable_qr", kwargs={"qr_id": location.location_code})
        )
        self.assertEqual(response.status_code, 200)
        location.refresh_from_db()
...
class GamekeeperAddTests(TestCase):
    ...
    def test_add_item_to_shop(self):
        ...
        os.remove("media/shop/shop_item.png")
    ...
    def test_add_points(self):
        ...
        response = self.client.get(
            reverse(
                "add_points",
                kwargs={"type": "bingame", "user_id": user.id, "amount": 10},
            )
        )
        self.assertEqual(response.status_code, 200)
