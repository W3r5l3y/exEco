...
def open_lootbox(request, lootbox_id):
    ...
    lootbox_contents = list(
        LootboxContents.objects.filter(lootbox_template=lootbox.lootbox_template)
    )
    ...
    return JsonResponse(
        {
            ...
            "item_won": {
                "name": selected_item.name,
                "image": (
                    request.build_absolute_uri(selected_item.image.url)
                    if selected_item.image
                    else None
                ),
                "item_type": "regular",
            },
            "lootbox_removed": lootbox.quantity
            == 0,  # Tell frontend if lootbox is gone
        }
    )
...
def merge_item(request, item_id):
    ...
    if not lootbox_template:
        return JsonResponse(
            {"error": "No corresponding lootbox found for this item"}, status=400
        )
    ...
    return JsonResponse(
        {
            "success": True,
            "message": "Merge successful, lootbox added to your inventory.",
        }
    )
