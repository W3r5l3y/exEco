...
class Inventory(models.Model):
    ...
    def addLootbox(self, lootbox_template, quantity=1):
        ...
        lootbox, created = InventoryItem.objects.get_or_create(
            ...
            defaults={
                "image": lootbox_template.lootbox_image,
                "item_type": "lootbox",
                "quantity": quantity,
                "lootbox_template": lootbox_template,
            },
        )
        if not created:
...
class LootboxTemplate(models.Model):
    ...
    lootbox_image = models.ImageField(
        upload_to="inventory/lootboxes/", blank=True, null=True
    )
...
class LootboxContents(models.Model):
    lootbox_template = models.ForeignKey(
        LootboxTemplate, on_delete=models.CASCADE, related_name="contents"
    )
...
class InventoryItem(models.Model):
    ...
    item_type = models.CharField(
        max_length=10, choices=ItemType.choices, default=ItemType.REGULAR
    )
    description = models.TextField(blank=True, null=True)
    image = models.ImageField(upload_to="inventory/items/", blank=True, null=True)
    ...
    lootbox_template = models.ForeignKey(
        LootboxTemplate, on_delete=models.SET_NULL, null=True, blank=True
    )
