...
def challenges_view(request):
    ...
    if not UserChallenge.objects.filter(
        ...
        all_lifetime_challenges = list(
            Challenge.objects.filter(challenge_type="lifetime")
        )
    ...
    lifetime_challenges = UserChallenge.objects.filter(
        user=user, challenge__challenge_type="lifetime"
    )
    ...
    if request.method == "POST":
        try:
            ...
            if not request.user.is_authenticated:
                return JsonResponse(
                    {"success": False, "error": "User is not logged in."}
                )
            ...
            if user_challenge.completed:
                return JsonResponse(
                    {"success": False, "message": "Challenge already completed."}
                )
