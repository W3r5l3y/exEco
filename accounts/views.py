    ...
    if request.method == "POST":
            ...
            if login_form.is_valid():
                ...
                if user is not None:
                    ...
                    if not user.profile_picture:
                        profile_pic_path = generate_profile_picture(
                    ...
                    garden_image_path = os.path.join(
                        settings.MEDIA_ROOT,
                        "gardens",
                        f"garden_state_user{user.id}.png",
                    )
        ...
        # ----- REGISTER -----
            ...
            if register_form.is_valid():
                ...
                create_empty_garden_image(
                    new_user
                )  # Create an empty garden image for the user
                profile_pic_path = generate_profile_picture(
                    new_user.first_name, new_user.last_name, new_user.email, new_user.id
...
def settings_view(request):
    ...
    if request.method == "POST":
        ...
        if "confirm_profile" in request.POST:
            ...
            if form.is_valid():
                form.save()
                return HttpResponseRedirect(
                    reverse("settings") + "?success=Profile changed successfully"
                )
        ...
        # ----- Change password -----
            ...
            if form.is_valid():
                ...
                if user is not None:
                    login(request, user)
                return HttpResponseRedirect(
                    reverse("settings") + "?success=Password changed successfully"
                )
            else:
                error_message = list(form.errors.values())[0][0]
                return HttpResponseRedirect(
    ...
    strava_linked = StravaToken.objects.filter(
        expires_at__gt=date.today(), user_id=user.id
    ).exists()
...
def log_out(request):
    ...
    try:
        ...
        return HttpResponseRedirect(
            reverse("settings") + "?error=You must be logged in to delete your account."
        )
...
def delete_account(request):
    ...
    try:
        ...
        return HttpResponseRedirect(
            reverse("settings")
            + f"?error=Failed to delete account. Please contact support."
        )
...
def request_gdpr(request):
    ...
    try:
        if request.user.is_authenticated:
            ...
            gdpr_text = f"------------------------------------------------------------\n     EXECO\n------------------------------------------------------------\n"
            gdpr_text += (
                f"Dear {firstName}, please see the data we have stored on you:\n"
            )
            gdpr_text += f"\n--------------------------------------------------\n     Account Section\n--------------------------------------------------\n"
            ...
            with open(zip_filename, "rb") as f:
                ...
                response["Content-Disposition"] = (
                    f"attachment; filename={firstName}_{lastName}_{date_now}.zip"
                )
            ...
            return HttpResponseRedirect(
                reverse("settings")
                + "?error=You must be logged in to request your gdpr."
            )
    except Exception as e:
        print(e)
        return HttpResponseRedirect(
            reverse("settings")
            + "?error=Failed to request gdpr. Please contact support."
        )
...
def strava_unlink(request):
    ...
    try:
        ...
        if user.is_authenticated:
            ...
            return HttpResponseRedirect(
                reverse("settings") + f"?success=Strava unlinked successfully"
            )
        else:
            return HttpResponseRedirect(
                reverse("settings")
        ...
        return HttpResponseRedirect(
            reverse("settings")
            + f"?error=Failed to unlink Strava. Please contact support."
        )
