...
def buy_item(request, item_id):
    ...
    if not user_coins.spend_coins(shop_item.cost):
        return JsonResponse(
            {"error": "Failed to spend coins - not enough coins in inventory"},
            status=500,
        )
    ...
    if shop_item.image:
        ...
        new_image_path = os.path.join(
            new_image_dir, os.path.basename(shop_item.image.name)
        )
        ...
        new_image_relative_path = (
            f"inventory/items/{os.path.basename(shop_item.image.name)}"
        )
    ...
    inventory.addItem(
        shop_item.name, new_image_relative_path, item_type="regular", stats=stats
    )
